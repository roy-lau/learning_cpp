...
根据哈希函数直接计算出某元素对应的哈希值，并将元素放入对应位置的桶中。关于哈希需要注意常见的哈希函数与常见的冲突解决方法。

考点：**装填因子(平均查找成功次数) = 表中元素数/表的容量上限（表长）**


## 图

![图的示意](./imgs/graph.png "图的示意")

图是由点和边组成的集合，边记录的是节点之间的关系。

- 图的基本术语：
1. 节点，边（狐）
2. 有向边，无相边。有向图，无向图。完全图，子图
3. 度，入度，出度
4. 路径，回路（环）
...
有向图：既有有向边又有无相边的图称为有向图，仅有有向边的图也称为有向图
完全图：从任意节点到任意节点都有边连接的图，称为完全图
子图：完全图的一部分称为子图

度：无向图的节点连着 n 条边，节点的度就是 n
入度：有向图的节点有 n 个有向边进入就是 n 个入度
出度：有向图的节点有 n 个有向边进入就是 n 个出度

连通图：一组(或一个)无向图组成的图，叫做连通图
强连通图：一组(或一个)有向图组成的图，叫做强连通图

连通分量：一组(或一个)无向图和其他无向图没有任何连接，叫做连通分量
强连通分量：一组(或一个)有向图和其他无向图没有任何连接，叫做强连通分量

有 n 个节点的连通图，最少有多少个边？ n-1
有 n 个节点的强连通图，最少有多少个边？ n
...
邻接矩阵是一个多维数组，存储节点间边的关系、设图的邻接矩阵为 A，`An` 的元素 `An[i][j]` 等于由顶点 i 到顶点 j 的长度为 n 的路径数目。

- 图的存储 -- 邻接表

```C
#define MaxCnt 50 // 节点最大数量
struct Node { // 边在链表中的节点
  int adjvex; // 该条边的终点
...
邻接表存储了与每个节点有关的所有边 与同一个节点有关的边都存到同一个链表中。

邻接矩阵优点是直观(x,y)，缺点是占用空间大(必须以x为起点)。
邻接表的优点是节省空间，缺点是不直观
...
从起点出发，每次走到一个新的点时，就以新的点位起点继续向下走。当走到不能走时，再进行回溯，深度优先遍历一般使用递归实现。

- 图的遍历 -- 广度优先

从起点出发，先遍历所有连通的点。接下来按照遍历的顺序，继续遍历每个点连通的点，广度优先遍历一般使用队列实现。

### 最小生成树（MST）

对于一个图来说，删掉一些边，使得每两个节点之间只有一条路径。那么，删掉边以后，可以得到一棵树，这棵树就叫做该图的最小生成树。所有生成树中，所有边权相加后，总权值最小的生成树，叫做最小生成树。

- 最小生成树的性质：
1. 若图中有 N 个节点，那么生成树中应该有 N-1 条边
2. 最小生成树的权值总和唯一，但最小生成树不一定唯一

- 最小生成树算法 -- kruskal 算法（克鲁斯卡尔算法）（以边为基准）
1. 先对边进行排序
2. 从小到大去遍历

- 最小生成树算法 -- prim 算法（普里姆算法）（以点为基准）
1. 选个起点
2. 选权值最小的向外延伸的边
...
对于从节点 A 到节点 B 的所有路径，总权值最小的路径称为从 A 到 B 的最短路径，简称最短路。最短路不一定唯一，但最短权值唯一。

- 最短路径求解方法 -- Dijkstra 算法（单源最短路）
...
1. 从所有有答案且答案未被确定的点中选一个答案最小的点
2. 遍历以该点位起点的所有边，通过这条边去更新 边的终点答案

- 最短路径求解方法 -- Floyd 算法（多源最短路）
...
用有向图表示一个工程，其节点表示活动有向边 1->5 表示 1 活动必须先于 5 活动这样一种关系，则称这个图为 AOV 网(顶点表示活动的网络)

### 拓扑排序：

对于 AOV 网来说，找到所有活动的一种排序，使得按照排序序列进行活动时，不违反网中活动先后顺序，在求解过程中通常使用入度计数来实现。

1. 求每个点的入度数量
2. 找出入度数量为 0 的节点
3. 遍历以该点为起点的所有边
4. 这些边的终点的入度数量减一
5. 重复 2 到 4 步骤，直到所有入度数量为0 

使用场景：拓扑排序可以判断图中是否有环
时间复杂度：O(n^2)


### AOE 网（边表示活动的网络）与关键路径

用有向图表示一个工程，其边表示活动有向边 1->5 的权值 3 表示该活动的开销（通常为活动所需时间），则称这个图为 AOE 网（边表示活动的网络）

- AOE 网与关键路径的性质
1. AOE 网只有一个入度为 0 的点（源点），表示工程的开始
2. AOE 网只有一个出度为 0 的点（汇点），表示工程的结束
3. 只有在某点所代表的事情发生以后，从该点出发的活动（有向边）才能开始
4. 只有在进入某点的各个活动都已经结束时，该点的事件才能发生
5. 从源点到汇点的最长路径称为关键路径，完成整个工程的最短事件就是关键路径的长度
